...
import sys
import sample_lhe
import read_lhe_2e2m
import ROOT as R
import argparse
import anaconfig

def main():
    R.gSystem.Load("/publicfs/cms/user/mingxuanzhang/gridpack/simulation_tool/mg5condor/ExRootAnalysis/libExRootAnalysis.so") 
    ...
    filelist, sampleID = sample_lhe.getfilelist(_process_name)
    chain, basic_weight, event_number = sample_lhe.getchain(_process_name, 'LHEF', filelist, _ifile_list)
    ...
    if '2e2m' in _process_name:
        ...
        ana = read_lhe_2e2m.analysis_gg2e2m(chain, sampleID, event_number, basic_weight, _ofile_name)
